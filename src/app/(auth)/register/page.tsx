  ...
  return (
    <div className="flex min-h-screen bg-white">
      ...
      <div className="flex w-full flex-col justify-center px-8 lg:w-1/2 xl:px-24">
        <div className="mx-auto w-full max-w-sm">
          ...
          <div className="mb-8 flex flex-col items-center justify-center text-center">
            ...
            <p className="mt-2 text-xs text-slate-400">Mulai langkah sukses bisnis Anda hari ini.</p>
          </div>

          {/* Tombol Google - Konsisten dengan Halaman Login */}
          <div className="mb-6">
            <Button 
              type="button"
              variant="outline" 
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="relative">
              ...
              <Input 
                type="password" placeholder="Create strong password" 
                className="pl-11 h-12 rounded-full border-slate-200 focus:ring-rose-500"
            ...
            <Button className="w-full h-12 rounded-full bg-rose-500 hover:bg-rose-600 font-bold shadow-lg shadow-rose-100 transition-all" type="submit" disabled={isLoading}>
              {isLoading ? <Loader2 className="animate-spin h-5 w-5" /> : "SIGN UP NOW"}
            </Button>
          </form>

          <div className="mt-8 text-center">
            <p className="text-sm text-slate-500">
              Already have an account? <Link href="/login" className="font-bold text-rose-500 hover:underline">Sign In</Link>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="hidden w-1/2 lg:block relative overflow-hidden bg-gradient-to-br from-[#0F172A] to-[#0D9488]">
          ...
          <h2 className="text-4xl font-black mb-4 uppercase tracking-tight">GABUNG SEKARANG</h2>
          <p className="text-slate-300 max-w-md text-lg">Kelola stok produk, pantau transaksi, dan dapatkan insight bisnis yang belum pernah Anda bayangkan sebelumnya.</p>
        </div>
      </div>
    </div>
  )
